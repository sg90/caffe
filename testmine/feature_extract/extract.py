...
def main(argv):
    ...
    parser.add_argument(
        ...
        default=os.path.join(pycaffe_dir,
                "senti/model_new.prototxt"),
        help="Model definition file."
    )
    parser.add_argument(
        "--pretrained_model",
        default=os.path.join(pycaffe_dir,
                "senti/pretrained_model"),
    ...
    for aline in fid:
        ...
        try:
            ...
            print 'URL Image open error: %s' % err
        ...
        for j in range(fc7.shape[1]):
            fout.write(str(fc7[0][j]) + ' ')
        fout.write('\n')
        if idnum % 100 == 0:
            print "%d inputs have finished." % idnum
            print "Time has gone %.2f s." % (time.time() - start)
            fout.flush()
